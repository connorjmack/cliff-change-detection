...
# Add pipeline directory to path (repo_root/code/pipeline)
REPO_ROOT = Path(__file__).resolve().parents[2]
PIPELINE_DIR = REPO_ROOT / "code" / "pipeline"
sys.path.insert(0, str(PIPELINE_DIR))

def load_module():
    """Load step 0 module fresh for each test."""
    import importlib.util
    spec = importlib.util.spec_from_file_location(
        "make_survey_lists",
        PIPELINE_DIR / "0_make_survey_lists.py"
    )
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module

def apply_mock_paths(module, root_str):
    """
    Point module paths to the temporary mock root.
    """
    root = Path(root_str)
    module.ROOT_LIDAR = str(root)
    module.instrument_paths = {
        "MiniRanger_Truck": str(root / "MiniRanger_Truck" / "LiDAR_Processed_Level2"),
        "MiniRanger_ATV": str(root / "MiniRanger_ATV" / "LiDAR_Processed_Level2"),
        "VMQLZ_Truck": str(root / "VMQLZ_Truck" / "LiDAR_Processed_Level2"),
        "VMZ2000_Truck": str(root / "VMZ2000_Truck" / "LiDAR_Processed_Level2"),
    }
...
class TestSurveyListGeneration:
    ...
    def mock_survey_structure(self, mock_lidar_root):
        ...
        for survey_name, instrument in surveys:
            ...
            # add dummy LAS so the new step-0 LAS check passes
            las_path = survey_path / f"{survey_name}_beach_cliff_ground.las"
    ...
    def test_export_surveys_creates_csv(self, mock_survey_structure):
        ...
        module = load_module()
        apply_mock_paths(module, mock_survey_structure)
    ...
    def test_no_surveys_found(self, mock_lidar_root):
        ...
        module = load_module()
        apply_mock_paths(module, mock_lidar_root)
